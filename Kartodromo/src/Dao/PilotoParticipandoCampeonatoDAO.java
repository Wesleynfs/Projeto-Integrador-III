...
public class PilotoParticipandoCampeonatoDAO implements GenericDAO<PilotoParticipandoCampeonato> {
    ...
    public List<PilotoParticipandoCampeonato> listarConvitesJaEnviadosPeloPiloto(Piloto piloto,Campeonato campeonato) throws Exception {
        try {
            entityManager = new ConnectionFactory().getConnection();
            return entityManager.createQuery("SELECT c FROM PilotoParticipandoCampeonato c where piloto = :p AND c.campeonato.situacao != 'Finalizado'").setParameter("p", piloto).getResultList();
        } catch (Exception e) {
    ...
    public PilotoParticipandoCampeonato Listar_o_piloto_de_campeonato(Piloto piloto, Campeonato campeonato) throws Exception {
        try {
            entityManager = new ConnectionFactory().getConnection();
            List<PilotoParticipandoCampeonato> list = entityManager.createQuery("SELECT c FROM PilotoParticipandoCampeonato c where c.piloto = :piloto AND c.campeonato = :campeonato")
                    .setParameter("piloto", piloto)
                    .setParameter("campeonato", campeonato)
                    .getResultList();
            return list.get(0);
